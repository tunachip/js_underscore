// src/engine/ecs/world/combat.js

import { ELEMENTS, STATUSES, STATUS_CAPS } from '../../../constants.js';
...
export function addEntity (combat, entity, isPlayer) {
	...
	for (const move of activeMoves) { addMove(combat, move, idx, 'active'); };
	for (const move of bankedMoves) { addMove(combat, move, idx, 'banked'); };
};
...
export function addMove (combat, move, ownerIndex, zone) {
	...
	const initIgnoresStatus = normalizeIgnoresStatus(move.ignoresStatus);
	combat.moveIgnoresStatus.push(initIgnoresStatus);
	const initIgnoresStatusTurnsLeft = normalizeIgnoresStatusTurnsLeft(move.ignoresStatus);
	combat.moveIgnoresStatusTurnsLeft.push(initIgnoresStatusTurnsLeft);
...
function normalizeIgnoresStatus (ignoresStatus = {}) {
	...
	for (const status of Object.keys(ignoresStatus)) {
		out[status] = Boolean(ignoresStatus[status][initialState]);
	};
	return out;
};

function normalizeIgnoresStatusTurnsLeft (ignoresStatus = {}) {
	const out = initStatusMap(0);
	for (const status of Object.keys(ignoresStatus)) {
		out[status] = Number(ignoresStatus[status][turns]);
	};
	return out;
};
