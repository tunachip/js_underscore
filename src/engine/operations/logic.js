// src/engine/operations/logic.js

import { Random } from '#engine/operations';
...
export function randVar (combat, key, type, range, secret) {
	switch (type) {
		case 'element': {
			const value = Random.element(range);
			setVar(combat, key, value, secret);
			return { break: false };
		};
		case 'status': {
			const value = Random.status(range);
			setVar(combat, key, value, secret);
			return { break: false };
		};
		case 'entity': {
			const value = Random.entity(combat, range);
			setVar(combat, key, value, secret);
			return { break: false };
		};
		case 'move': {
			const value = Random.move(combat, range);
			setVar(combat, key, value, secret);
			return { break: false };
		};
		default: {
...
export function encounterChoice (options) {
	return Random.int(0, options.length-1);
};
